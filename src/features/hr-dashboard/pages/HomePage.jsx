...
export default function HomePage() {
  ...
  return (
    <div className="h-screen flex flex-col px-4 py-2 space-y-3">
      {/* القسم الأول */}
      <div className="flex flex-col lg:flex-row gap-3 flex-none">
        <div className="flex-1 bg-[#E9E8E84D] rounded-xl p-3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <QuickInfoCard icon={icon1} title="عدد الموظفين" value={attendanceData ? attendanceData.totalEmployees : "--"} />
            <QuickInfoCard icon={icon2} title="الحاضرين" value={attendanceData ? attendanceData.present : "--"} />
            <QuickInfoCard icon={icon3} title="المتأخرين" value={attendanceData ? attendanceData.late : "--"} />
            <QuickInfoCard icon={icon4} title="الغائبين" value={attendanceData ? attendanceData.absentWithExcuse + attendanceData.absentWithoutExcuse : "--"} />
          </div>
        </div>

        <div className="relative w-full lg:w-1/3 rounded-xl p-3 shadow flex flex-col" style={{ backgroundColor: "#E9E8E84D" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-3 flex-none">
        {/* الإقامات */}
        <div className="relative lg:col-span-2 rounded-xl p-3 shadow flex flex-col text-xs" style={{ backgroundColor: "#E6E6E64D" }}>
         <div className="absolute top-0 left-0 w-[45px] h-[45px] bg-[#FF98310D] rounded-br-full"></div>
...
<rect width="24" height="24" fill="white" transform="translate(8 8.5)"/>
          ...
          <div className="grid grid-cols-2 gap-2 text-center flex-1 w-full mt-3">
            <div>
              ...
              <span className="font-bold">{stats ? stats.residencies.active : "--"}</span>
            </div>
            <div>
              <span className="text-[#410A5F] block">قرب الانتهاء</span>
              <span className="font-bold">{stats ? stats.residencies.expiringSoon : "--"}</span>
            </div>
          </div>
        </div>
...
<rect width="24" height="24" fill="white" transform="translate(8 8.5)"/>
          ...
          <div className="grid grid-cols-2 gap-2 text-center flex-1 w-full mt-3">
            <div>
              ...
              <span className="font-bold">{stats ? stats.contracts.active : "--"}</span>
            </div>
            <div>
              <span className="text-[#410A5F] block">قرب الانتهاء</span>
              <span className="font-bold">{stats ? stats.contracts.expiringSoon : "--"}</span>
            </div>
          </div>
        </div>

        {/* جدول السجلات */}
        <div className="relative lg:col-span-8 rounded-xl shadow p-2 overflow-y-auto  flex flex-col custom-scrollbar h-full min-h-0" style={{ backgroundColor: "#E9E8E84D" }}>
          <div className="absolute top-0 left-0 w-[60px] h-[60px] bg-[#FF98310D] rounded-br-full"></div>
          <CommercialTable />
        </div>
      </div>

      {/* القسم الثالث */}
   <div className="flex-1 min-h-[300px] grid grid-cols-1 lg:grid-cols-12 gap-3">
  <div className="col-span-1 lg:col-span-8 flex flex-col min-h-0">
    <div className=" relative bg-[#E6E6E64D] rounded-xl  p-2 flex-1 overflow-y-auto custom-scrollbar min-h-0">
      <div className="absolute top-0 left-0 w-[70px] h-[70px]  bg-[#410A5F05] rounded-br-full"></div>
      <RequestsTable />
    </div>
  </div>
